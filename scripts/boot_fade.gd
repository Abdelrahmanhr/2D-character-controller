...
## starts on a solid black screen, holds the official Godot logo for a few
...
func _ready() -> void:
	...
	fade_in.tween_property(godot_logo, "modulate:a", 1.0, fade_in_duration)
	await fade_in.finished
	await get_tree().create_timer(hold_duration).timeout
	var fade_out := create_tween()
	fade_out.tween_property(godot_logo, "modulate:a", 0.0, fade_out_duration)
